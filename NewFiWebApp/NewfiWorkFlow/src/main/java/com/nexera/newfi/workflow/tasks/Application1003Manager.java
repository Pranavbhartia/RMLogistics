...
@Component
public class Application1003Manager extends NexeraWorkflowTask
	...
	public String execute(HashMap<String, Object> objectMap) {
		String status = objectMap
		        .get(WorkflowDisplayConstants.WORKITEM_STATUS_KEY_NAME)
		        .toString();
		...
		if (status.equals(String
			...
			if (objectMap.get(
			        WorkflowDisplayConstants.INTERVIEW_DATE_KEY_NAME) == null) {
				return returnStatus;
			}
			Date interviewDate = utils.parseStringIntoDate(objectMap
			        .get(WorkflowDisplayConstants.INTERVIEW_DATE_KEY_NAME)
			        .toString());
	...
	public void sendEmail(HashMap<String, Object> objectMap, String subject) {
		if (objectMap != null) {
			LoanVO loanVO = loanService.getLoanByID(Integer.parseInt(
			        objectMap.get(WorkflowDisplayConstants.LOAN_ID_KEY_NAME)
			                .toString()));
			String emailTemplateKey = objectMap
			...
			if (loanVO != null && emailTemplateKey != null) {
				...
				if (template != null) {
					LOG.info(
					        "Send Email Template Found " + template.getValue());
				...
				if (loanManagerUsername != null) {
					loanManagerccList
					        .add(CommonConstants.SENDER_DEFAULT_USER_NAME + "-"
					                + loanManagerUsername + "-" + loanVO.getId()
					                + CommonConstants.SENDER_EMAIL_ID);
	...
	private void createAlertForDisclosureDue(
		...
		if (notificationList.size() == 0) {
			...
			WorkflowItemMaster workflowItemMaster = workflowService
			        .getWorkflowByType(
			                WorkflowConstants.WORKFLOW_ITEM_DISCLOSURE_STATUS);
