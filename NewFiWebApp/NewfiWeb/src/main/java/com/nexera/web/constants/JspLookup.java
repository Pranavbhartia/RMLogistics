package com.nexera.web.constants;

public interface JspLookup {

	public static final String LOGIN = "login";
	public static final String USER_LOGIN = "userlogin";
	public static final String CUSTOM = "display";
	public static final String ERROR = "error";
	public static final String CUSTOMER_VIEW = "customerViewTemplate";
	public static final String AGENT_VIEW = "agentViewTemplate";
	
	public static final String HOME_PAGE = "home";
	public static final String TEASER_RATE_CALC = "teaserRateCalc";
	public static final String CUSTOMER_UPLOAD_DOCUMENT = "customerUploadRequiredFiles";
	public static final String PAYMENT_PAGE="payment";

}
