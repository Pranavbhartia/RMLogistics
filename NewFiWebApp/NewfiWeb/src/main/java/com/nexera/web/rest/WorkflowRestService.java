...
public class WorkflowRestService
{
    ...
    @RequestBody String milestoneNotificationStr )
    {
        ...
        try {
            ...
            params[1] = milestoneNoticationVO.getNotificationVO();
            engineTrigger.startWorkFlowItemExecution( milestoneNoticationVO.getMilestoneId(), params );

            response = RestUtil.wrapObjectForSuccess( "Milestone in execution " );
            LOG.debug( "Response" + response );
        } catch ( Exception e ) {
            LOG.error( e.getMessage() );
            response = RestUtil.wrapObjectForFailure( null, "500", e.getMessage() );
