...
public class GeneratePdfForQuickQuote {
	...
	static class HeaderFooterPageEventForPdf extends PdfPageEventHelper {
		...
		public void onEndPage(PdfWriter writer, Document document) {
			...
			Phrase p = new Phrase();
			p.add(new Chunk(
			        "Newfi dba of Nexera Holding LLC  | NMLS ID 1231327  |  ",
			        FontFactory.getFont("Calibri", 6, Font.BOLD)));
			p.add(new Chunk(image, 0, 0));
			p.add(new Chunk("  Equal Housing Lender",
	...
	public void openPdf(ByteArrayOutputStream byteArrayOutputStream) {
		...
		try {
			...
			LOGGER.error("Error in opening pdf under quick quote, catch-2:",
			        ex);
		}
	}
	...
	public void generatePdf(GeneratePdfVO generatePdfVO,
	        HttpServletRequest httpServletRequest,
	        ByteArrayOutputStream byteOutput, UserVO user)
		...
		Font creditFont = FontFactory.getFont("Calibri", 6);
		...
		Font boldFontWithBigSize = FontFactory.getFont("Calibri", 10,
		        Font.BOLD);
		...
		try {
			...
			if (user.getPhotoImageUrl() != null) {
				bytes = s3FileUploadService
				        .getInputStreamOfFile(user.getPhotoImageUrl());
		...
		cell = new PdfPCell(new Phrase(
		        "\nLoan Estimate Prepared for " + customerFirstName + " "
		                + customerLastName + "\n\n    " + todaysDate,
		        boldFontWithBigSize));
		...
		imageFilePath = basePath + File.separator + "resources" + File.separator
		        + "images" + File.separator + "apply_button.png";
		File imageFilePathLocation = new File(imageFilePath);

		Image buttonImage = Image
		        .getInstance(imageFilePathLocation.getAbsolutePath());
		...
		String loanType = generatePdfVO.getInputCustmerDetailUnderQuickQuote()
		        .getLoanType();
		...
		String propertyType = generatePdfVO
		        .getInputCustmerDetailUnderQuickQuote().getPropertyType();
		...
		String propertyUse = generatePdfVO
		        .getInputCustmerDetailUnderQuickQuote().getResidenceType();
		...
		String purchasePrice = generatePdfVO
		        .getInputCustmerDetailUnderQuickQuote().getPurchaseDetails()
		        .getHousePrice();
		String loanAmount = generatePdfVO.getInputCustmerDetailUnderQuickQuote()
		...
		if (loanType.equals("PUR")) {
			...
			cell = new PdfPCell(new Phrase(
			        addDollarAndCommaWithoutDecimal(purchasePrice), font));
			cell.setBorder(Rectangle.RIGHT);
			cell.setHorizontalAlignment(Element.ALIGN_LEFT);
			firstTable.addCell(cell);
			...
			Float downPayment = Float.parseFloat(purchasePrice)
			        - Float.parseFloat(loanAmount);
			cell = new PdfPCell(new Phrase(
			        addDollarAndCommaWithoutDecimal("" + downPayment), font));
			cell.setBorder(Rectangle.RIGHT);
			cell.setHorizontalAlignment(Element.ALIGN_LEFT);
			firstTable.addCell(cell);
		}
		...
		if (loanType.equals("PUR")) {
			cell = new PdfPCell(new Phrase(
			        addDollarAndCommaWithoutDecimal(loanAmount), font));
		} else if (loanType.equals("REF")) {
			loanAmountDuringRefinance = generatePdfVO
			        .getInputCustmerDetailUnderQuickQuote()
			        .getCurrentMortgageBalance();
			cell = new PdfPCell(new Phrase(
			        addDollarAndCommaWithoutDecimal(loanAmountDuringRefinance),
			        font));
		} else {
			String cashOut = generatePdfVO
			        .getInputCustmerDetailUnderQuickQuote().getCashTakeOut();
			if (cashOut.contains("$")) {
				cashOut = removeDollarAndComma(cashOut);
			}

			Float cashOutLoanAmount = Float.parseFloat(cashOut)
			        + Float.parseFloat(
			                generatePdfVO.getInputCustmerDetailUnderQuickQuote()
			                        .getCurrentMortgageBalance());
			loanAmountDuringRefinance = "" + cashOutLoanAmount;
			cell = new PdfPCell(new Phrase(
			        addDollarAndCommaWithoutDecimal("" + cashOutLoanAmount),
			        font));
		}
		...
		if (!loanType.equals("PUR")) {
			cell = new PdfPCell(
			        new Phrase("  " + "Estimated Home Value", font));
			cell.setBorder(Rectangle.LEFT);
			firstTable.addCell(cell);

			homeWorthTodayDuringRefinance = generatePdfVO
			        .getInputCustmerDetailUnderQuickQuote().getHomeWorthToday();
			cell = new PdfPCell(new Phrase(addDollarAndCommaWithoutDecimal(
			        homeWorthTodayDuringRefinance), font));
			cell.setBorder(Rectangle.RIGHT);
			cell.setHorizontalAlignment(Element.ALIGN_LEFT);
			firstTable.addCell(cell);
		}
		...
		Float loanToValue;
		if (loanType.equals("PUR")) {
			loanToValue = (Float.parseFloat(loanAmount)
			        / Float.parseFloat(purchasePrice)) * 100;
		} else {
			loanToValue = (Float.parseFloat(loanAmountDuringRefinance)
			        / Float.parseFloat(homeWorthTodayDuringRefinance)) * 100;
		}
		...
		cell = new PdfPCell(
		        new Phrase("  " + "Estimated Monthly Payment", font));
		cell.setBorder(Rectangle.LEFT);
		firstTable.addCell(cell);
		...
		String teaserRate = generatePdfVO.getLqbTeaserRateUnderQuickQuote()
		        .getTeaserRate();
		...
		cell = new PdfPCell(
		        new Phrase("Estimated Cash from / to Borrower", boldFont));
		cell.setBorder(Rectangle.LEFT | Rectangle.RIGHT | Rectangle.TOP);
		// cell.setColspan(2);
		cell.setColspan(2);
		// cell.setPaddingTop(4);
		...
		if (loanType.equals("PUR")) {
			...
			cell = new PdfPCell(
			        new Phrase("  " + "Current Loan Balance", font));
		} else {
		...
		String currentMortgageBalance = generatePdfVO
		        .getInputCustmerDetailUnderQuickQuote()
		        .getCurrentMortgageBalance();
		...
		if (loanType.equals("PUR")) {
			...
			cell = new PdfPCell(new Phrase(
			        addDollarAndCommaWithoutDecimal(currentMortgageBalance),
			        font));
			...
			Float cashOutLoanAmount = Float.parseFloat(cashOut)
			        + Float.parseFloat(currentMortgageBalance);
			newLoanBalancePrice = addDollarAndCommaWithoutDecimal(
			        "" + cashOutLoanAmount);
			cell = new PdfPCell(new Phrase(newLoanBalancePrice, font));
		}

		String totEstimatedClosingCost = generatePdfVO
		        .getLqbTeaserRateUnderQuickQuote().getTotEstimatedClosingCost();
		if (totEstimatedClosingCost != null) {
			totEstimatedClosingCost = removeDollarAndComma(
		...
		if (loanType.equals("PUR") || loanType.equals("REF")) {
			cell = new PdfPCell(
			        new Phrase("  " + "+Estimated Closing Costs", font));
		} else {
			cell = new PdfPCell(
			        new Phrase("  " + "-Estimated Closing Costs", font));
		}
		...
		String totEstThdPtyCst = generatePdfVO.getLqbTeaserRateUnderQuickQuote()
		        .getTotEstThdPtyCst();
		String TotEstLenCost = generatePdfVO.getLqbTeaserRateUnderQuickQuote()
		        .getTotEstLenCost();

		boolean isEstClosingCostNegative = false;
		if (Float.parseFloat(removeDollarCommaAndBracket(TotEstLenCost)) > Float
		        .parseFloat(removeDollarCommaAndBracket(totEstThdPtyCst))) {
		...
		if (estimatingClosingCost.equals("$0"))
			...
			if (estimatingClosingCost.contains("$")
			        && !(estimatingClosingCost.contains("("))) {
				cell = new PdfPCell(new Phrase(estimatingClosingCost, font));
			} else if (!(estimatingClosingCost.contains("$"))
			        && (estimatingClosingCost.contains("("))) {
				cell = new PdfPCell(new Phrase(
				        addDollarAndComma(removeDollarCommaAndBracket(
				                "" + estimatingClosingCost)),
				        font));
			} else {
				cell = new PdfPCell(new Phrase(
				        addDollarAndComma("" + estimatingClosingCost), font));
			}
		...
		String totEstResDepWthLen = generatePdfVO
		        .getLqbTeaserRateUnderQuickQuote().getTotEstResDepWthLen();
		...
		if (totEstimatedClosingCost.contains("(")) {
			estimatedNumber = removeDollarCommaAndBracket(
			        totEstimatedClosingCost);
		...
		if (totEstResDepWthLen.contains("(")) {
			prepaidAgainstNumber = removeDollarCommaAndBracket(
			        totEstResDepWthLen);
		} else if (totEstimatedClosingCost.contains("$")) {
		...
		// }
		if (isEstClosingCostNegative) {
			if (Float.parseFloat(estimatedNumber) > Float
			        .parseFloat(removeDollarAndComma(totEstResDepWthLen))) {

			} else if (Float.parseFloat(estimatedNumber) < Float
			        .parseFloat(removeDollarAndComma(totEstResDepWthLen))) {
				Float prepaidNumber = Float.parseFloat(
				        removeDollarAndComma(totEstResDepWthLen)) - (Float
				                .parseFloat(removeDollarAndComma(
				                        totEstResDepWthLen))
				                - Float.parseFloat(estimatedNumber));
				prepaidComment = addDollarAndComma("" + prepaidNumber);

				Float estimatedPrepaidNumber = Float
				        .parseFloat(removeDollarAndComma(totEstResDepWthLen))
				        - Float.parseFloat(estimatedNumber);
				estimatedPrepaids = addDollarAndComma(
				        "" + estimatedPrepaidNumber);
			} else {
		...
		if (loanType.equals("PUR")) {
			if (!loanAmount.contains("$")) {
				...
				cell = new PdfPCell(
				        new Phrase(
				                addDollarAndCommaWithoutDecimal(
				                        removeDollarAndComma(loanAmount)),
				                font));
			}
		} else {
			if (!currentMortgageBalance.contains("$")) {
				...
				cell = new PdfPCell(new Phrase(
				        addDollarAndCommaWithoutDecimal(
				                removeDollarAndComma(currentMortgageBalance)),
				        font));
			}
		}
		...
		String lenderFee813 = generatePdfVO.getLqbTeaserRateUnderQuickQuote()
		        .getLenderFee813();
		String creditOrCharge802 = generatePdfVO
		        .getLqbTeaserRateUnderQuickQuote().getCreditOrCharge802();
		if (Float.parseFloat(removeDollarCommaAndBracket(lenderFee813)) < Float
		        .parseFloat(removeDollarCommaAndBracket(creditOrCharge802))) {
		...
		if (loanType.equals("PUR")) {
			Float cashFromToBorrowerNumber = Float.parseFloat(
			        removeDollarCommaAndBracket(newLoanBalancePrice))
			        + Float.parseFloat(
			                removeDollarCommaAndBracket(estimatingClosingCost))
			        + Float.parseFloat(
			                removeDollarCommaAndBracket(estimatedPrepaids))
			        - Float.parseFloat(removeDollarCommaAndBracket(loanAmount));
			cashFromToBorrower = addDollarAndComma(
			        "" + cashFromToBorrowerNumber);
		} else if (loanType.equals("REF")) {
			Float cashFromToBorrowerNumber = Float.parseFloat(
			        removeDollarCommaAndBracket(newLoanBalancePrice))
			        + Float.parseFloat(
			        ...
			        + Float.parseFloat(
			                removeDollarCommaAndBracket(estimatedPrepaids))
			        - Float.parseFloat(removeDollarCommaAndBracket(
			                loanAmountDuringRefinance));
			cashFromToBorrower = addDollarAndComma(
			        "" + cashFromToBorrowerNumber);
		} else {
			Float cashFromToBorrowerNumber = Float.parseFloat(
		...
		if (showPrepaidComment) {
			...
			cell = new PdfPCell(
			        new Phrase(
			                "  *A credit of " + prepaidCost
			                        + " was applied against the "
			                        + prepaidAgainstNumber + " in prepaids.",
			                creditFont));
			// cell.setBorder(Rectangle.LEFT | Rectangle.RIGHT);
			cell.setColspan(2);
		...
		// String lenderFee813 =
		// generatePdfVO.getLqbTeaserRateUnderQuickQuote().getLenderFee813();
		...
		cell = new PdfPCell(new Phrase(
		        "  " + "Your cost or credit based on rate selected", font));
		cell.setPaddingTop(4);
		cell.setPaddingBottom(4);
		cell.setBorder(Rectangle.LEFT);
		secondTable.addCell(cell);

		// String creditOrCharge802 =
		// generatePdfVO.getLqbTeaserRateUnderQuickQuote().getCreditOrCharge802();
		...
		if (isLenderCostNegative) {
			if (TotEstLenCost.contains("$") && TotEstLenCost.contains("(")) {
				...
				if (TotEstLenCost.contains("$")
				        && !(TotEstLenCost.contains("("))) {
					cell = new PdfPCell(
					        new Phrase("(" + TotEstLenCost + ")", font));
				} else if (!(TotEstLenCost.contains("$"))
				        && (TotEstLenCost.contains("("))) {
					cell = new PdfPCell(new Phrase("("
					        + addDollarAndComma(
					                removeDollarCommaAndBracket(TotEstLenCost))
					        + ")", font));
				} else {
					cell = new PdfPCell(new Phrase(
					        "(" + addDollarAndComma(TotEstLenCost) + ")",
					        font));
				}
			}
		} else {
			...
			if (TotEstLenCost.contains("(")) {
				cell = new PdfPCell(new Phrase(
				        TotEstLenCost.substring(1, TotEstLenCost.length() - 1),
				        font));
			} else if (!(TotEstLenCost.contains("$"))) {
				cell = new PdfPCell(
				        new Phrase(addDollarAndComma(TotEstLenCost), font));
			} else {
				cell = new PdfPCell(new Phrase(TotEstLenCost, font));
			}
		...
		cell = new PdfPCell(
		        new Phrase("Estimated Third Party Costs", boldFont));
		...
		String appraisalFee804 = generatePdfVO.getLqbTeaserRateUnderQuickQuote()
		        .getAppraisalFee804();
		...
		String creditReport805 = generatePdfVO.getLqbTeaserRateUnderQuickQuote()
		        .getCreditReport805();
		...
		String floodCertification807 = generatePdfVO
		        .getLqbTeaserRateUnderQuickQuote().getFloodCertification807();
		...
		String wireFee812 = generatePdfVO.getLqbTeaserRateUnderQuickQuote()
		        .getWireFee812();
		...
		String ownerTitleInsurance = generatePdfVO
		        .getLqbTeaserRateUnderQuickQuote()
		        .getOwnersTitleInsurance1103();
		...
		String lendersTitleInsurance1104 = generatePdfVO
		        .getLqbTeaserRateUnderQuickQuote()
		        .getLendersTitleInsurance1104();
		...
		String closingEscrowFee1102 = generatePdfVO
		        .getLqbTeaserRateUnderQuickQuote().getClosingEscrowFee1102();
		...
		String recordingFees1202 = generatePdfVO
		        .getLqbTeaserRateUnderQuickQuote().getRecordingFees1202();
		if (!recordingFees1202.contains("$")) {
			cell = new PdfPCell(
			        new Phrase(addDollarAndComma(recordingFees1202), font));
		} else {
		...
		String notaryfee1110 = generatePdfVO.getLqbTeaserRateUnderQuickQuote()
		        .getNotaryfee1110();
		if (!notaryfee1110.contains("$")) {
			cell = new PdfPCell(
			        new Phrase(addDollarAndComma(notaryfee1110), font));
		} else {
		...
		cell = new PdfPCell(
		        new Phrase("  " + "City/County Transfer Taxes", font));
		cell.setPaddingTop(4);
		cell.setPaddingBottom(4);
		...
		String cityCountyTax = generatePdfVO.getLqbTeaserRateUnderQuickQuote()
		        .getCityCountyTaxStamps1204();
		...
		cell = new PdfPCell(
		        new Phrase("Total Estimated Third Party Costs", font));
		// cell.setPaddingTop(4);
		// cell.setPaddingBottom(4);
		...
		// String totEstThdPtyCst =
		// generatePdfVO.getLqbTeaserRateUnderQuickQuote().getTotEstThdPtyCst();
		...
		cell = new PdfPCell(
		        new Phrase("Total Estimated Closing Costs", boldFont));
		// cell.setPaddingTop(4); cell.setPaddingBottom(4);
		...
		if (isTotalEstClosingCostNegative) {
			if (totEstimatedClosingCost.contains("$")
			        && totEstimatedClosingCost.contains("(")) {
				cell = new PdfPCell(new Phrase(totEstimatedClosingCost, font));
			} else {
				if (totEstimatedClosingCost.contains("$")
				        && !(totEstimatedClosingCost.contains("("))) {
					cell = new PdfPCell(new Phrase(
					        "(" + totEstimatedClosingCost + ")", font));
				} else if (!(totEstimatedClosingCost.contains("$"))
				        && (totEstimatedClosingCost.contains("("))) {
					cell = new PdfPCell(
					        new Phrase("("
					                + addDollarAndComma(
					                        removeDollarCommaAndBracket(
					                                totEstimatedClosingCost))
					                + ")", font));
				} else {
					cell = new PdfPCell(
			...
			if (totEstimatedClosingCost.contains("(")) {
				cell = new PdfPCell(
				        new Phrase(
				                totEstimatedClosingCost.substring(1,
				                        totEstimatedClosingCost.length() - 1),
				        font));
			} else if (!(totEstimatedClosingCost.contains("$"))) {
				cell = new PdfPCell(new Phrase(
				        addDollarAndComma(totEstimatedClosingCost), font));
			} else {
				cell = new PdfPCell(new Phrase(totEstimatedClosingCost, font));
			}
		...
		// String totEstimatedClosingCost =
		// generatePdfVO.getLqbTeaserRateUnderQuickQuote().getTotEstimatedClosingCost();
		// if(totEstimatedClosingCost.contains("(")){
		// totEstimatedClosingCost = totEstimatedClosingCost.substring(1,
		// totEstimatedClosingCost.length()-1);
		// }
		// if(totEstimatedClosingCost.contains("("))
		// cell = new PdfPCell(new
		// Phrase(addDollarAndComma(totEstimatedClosingCost), font));
		// else
		// cell = new PdfPCell(new
		// Phrase(addDollarAndComma(totEstimatedClosingCost), font));
		// cell.setPaddingTop(4); cell.setPaddingBottom(4);
		...
		if (impounds.equals("No")) {
			...
			String interest901 = generatePdfVO.getLqbTeaserRateUnderQuickQuote()
			        .getInterest901();
			...
			//
			// String hazIns903 =
			// generatePdfVO.getLqbTeaserRateUnderQuickQuote().getHazIns903();
			...
			// cell.addElement(new Phrase(" "+"Varies based on calendar month of
			// closing",font));
			cell.setPaddingTop(4);
			cell.setPaddingBottom(4);
			...
			String taxResrv1004 = generatePdfVO
			        .getLqbTeaserRateUnderQuickQuote().getTaxResrv1004();
			...
			cell = new PdfPCell();
			cell.addElement(
			        new Phrase("  " + "Homeowners Insurance Reserve", font));
			// cell.addElement(new Phrase(" "+"Provided you have 6 months of
			// remaining coverage",font));
			// cell.addElement(new Phrase(" "+"Note: Taxes for 1st and 2nd
			// installments must be paid or",font));
			// cell.addElement(new Phrase(" "+"will be collected at
			// closing.",font));
			cell.setPaddingTop(4);
			cell.setPaddingBottom(4);
			cell.setBorder(Rectangle.LEFT);
			...
			String hazInsReserve1002 = generatePdfVO
			        .getLqbTeaserRateUnderQuickQuote().getHazInsReserve1002();
			...
			// String totEstResDepWthLen =
			// generatePdfVO.getLqbTeaserRateUnderQuickQuote().getTotEstResDepWthLen();
			...
			// cell = new PdfPCell();
			cell = new PdfPCell(new Phrase(
			        "  " + "Tax Reserve - Estimated 6 Months", font));
			// cell.addElement(new Phrase(" "+"Tax Reserve - Estimated 6
			// Months",font));
			// cell.addElement(new Phrase(" "+"Varies based on calendar month of
			// closing",font));
			cell.setPaddingTop(4);
			...
			cell = new PdfPCell(new Phrase(
			        "  " + "Homeowners Insurance Reserve - Estimated 6 Months",
			        font));
			// cell.addElement(new Phrase(" "+"Homeowners Insurance Reserve -
			// Estimated 6 Months",font));
			// cell.addElement(new Phrase(" "+"Provided you have 6 months of
			// remaining coverage",font));
			// cell.addElement(new Phrase(" "+"Note: Taxes for 1st and 2nd
			// installments must be paid or",font));
		...
		document.add(horizontalLine);
		document.add(new Phrase("\nRATE QUOTE ASSUMPTIONS AND DISCLOSURES\n",
		        boldFooterFont));
		document.add(new Chunk(
		        "" + "This is only an estimate and not a commitment to lend. Rate displayed are subject to change. On adjustable-rate loans, interest rates are subject to potential increases over the life of the loan, once the initial "
		                ...
		                + "Call newfi for details at 888-316-3934.",
		        footerFont));
	...
	public String sendPurchasePdf(GeneratePdfVO generatePdfVO,
	        HttpServletRequest request) {
		String s3Path = "";
		try {
			UserVO user = userProfileService
			        .findUser(generatePdfVO.getUserId());
			...
			// UserVO user =
			// userProfileService.findUser(generatePdfVO.getUserId());
			...
			LOGGER.error("Error in generating purchase pdf under quick quote:",
			        ex);
		} finally {
			return s3Path;
		}
	...
	public String writeAndUploadFile(ByteArrayOutputStream byteOutStream)
	        throws IOException {
	...
	public void sendPurchasePdfEmail(UserVO userVo,
		...
		template = templateService.getTemplateByKey(
		        CommonConstants.TEMPLATE_KEY_NAME_PURCHASE_PDF);

		// We create the substitutions map
		Map<String, String[]> substitutions = new HashMap<String, String[]>();
		substitutions.put("-name-", new String[] {
		        userVo.getFirstName() + " " + userVo.getLastName() });
		substitutions.put("-custname-",
		        new String[] { generatePdfVO.getFirstName() + " "
		                + generatePdfVO.getFirstName() });
		...
		emailEntity.setFileName(CommonConstants.FILE_NAME_PURCHASE_PDF
		        + generatePdfVO.getFirstName() + "_"
