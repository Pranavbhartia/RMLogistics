...
function validateMortage()
{
	...
	{				
		$('.ce-input').next('.err-msg').html("Property value can not be less than requested loan amount " + refinanceTeaserRate.currentMortgagePayment).show();
		$('.ce-input').addClass('ce-err-input').show();
	}
	else
	{
...
function getLoanSummaryContainerPurchaseCEP(teaserRate, customerInputData) {
    ...
    */
    ...
    var bottomRcRow = getLoanSummaryLastRow("Total Est.<br/>Monthly Payment ", showValue(rateVO[index].payment) ,"totalEstMonthlyPaymentId");
    bottomRightCol.append(bottomRcRow);
