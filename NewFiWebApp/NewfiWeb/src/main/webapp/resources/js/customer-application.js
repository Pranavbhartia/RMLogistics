...
function getContextApplicationSelectQues(contxt) {
    ...
    for (var i = 0; i < contxt.options.length; i++) {
        ...
        optionCont.bind('click', {
            ...
            $(this).closest('.app-options-cont').find('.app-option-selected').html($(this).html());
            $(this).closest('.app-options-cont').find('.app-option-selected').data("value", $(this).data("value"));
...
function getApplicationSelectQues(question) {
    ...
    for (var i = 0; i < question.options.length; i++) {
        ...
        var optionCont = $('<div>').attr({
            ...
            "value": option.value
        }).html(option.text).on('click', function() {
            $(this).closest('.app-options-cont').find('.app-option-selected').html($(this).html());
            $(this).closest('.app-options-cont').find('.app-option-selected').data("value", $(this).data("value"));
            $(this).closest('.app-dropdown-cont').toggle();
        });
...
function paintSecondPropertyStep() {
    ...
    var saveAndContinueButton = $('<div>').attr({
        ...
        if (isSecondaryMortgage == 'Yes') appUserDetails.secondMortgage = true;
        else {
...
function paintMyIncome() {
    ...
    if (purchase == true) {
        var paintSaleOfCurrentHomeDIV = paintSaleOfCurrentHome();
...
function paintCustomerApplicationPageStep5() {
    ...
    if (dob == "" || dob == undefined || dob == 'NaN/NaN/NaN') dob = "";
    var questions = [{
        type: "desc",
        text: "Birthday",
        name: "birthday",
...
function appProgressBaar(num) {
    var count = 6;
    $("#appProgressBaarId_" + num).removeClass('ce-lp-in-progress').removeClass('ce-lp-complete').addClass('ce-lp-in-progress');
    $('#appStepNoId_' + num).html(num);
    for (var i = 1; i <= num - 1; i++) {
        $("#appProgressBaarId_" + i).removeClass('ce-lp-in-progress').removeClass('ce-lp-not-started').addClass('ce-lp-complete');
        $('#appStepNoId_' + i).html("");
    }
    for (var i = num + 1; i <= count; i++) {
        $("#appProgressBaarId_" + i).removeClass('ce-lp-in-progress').removeClass('ce-lp-complete').addClass('ce-lp-not-started');
