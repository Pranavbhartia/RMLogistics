...
function changeLeftPanel(primary, callback) {
	...
	var contxt = getNotificationContext(newfiObject.user.defaultLoanId,
			newfiObject.user.id);
...
function findUser(callback) {
	ajaxRequest("rest/userprofile/completeprofile", "GET", "json", {},
			function(response) {
				appendCustPersonalInfoWrapper(response, callback)
			});
}
/*
 * var logedInUser;
...
function getCustomerSecondaryLeftNav() {
	...
	if (newfi.appUserDetails) {
		try {
			...
			if (tutorialStatus && tutorialStatus == true
...
function paintApplicationAlreadySubmittedPage() {
	$('#center-panel-cont')
			.html(
					"<div class='complete-application-header'>Application already submitted.</div>");
	...
	var descText = $('<div>')
			...
			.html(
					"Congratulations! Based on the information you provided, you have been pre-qualified for a newfi loan. Here's what next:");
	...
	$('#center-panel-cont').append(descText).append(btn1).append(btn2).append(
			btn3);
}
...
function paintGettingToKnowPage() {
	...
	/*
	 * var header = $('<div>').attr({ "class": "complete-application-header"
	 * }).html("Getting to Know Newfi").on('click',function(){
	 * 
	 * var inputData = {}; inputData.id =
	 * JSON.parse(newfiObject.appUserDetails).user.customerDetail.id;
	 * inputData.loanId = JSON.parse(newfiObject.appUserDetails).loan.id;
	 * 
	 * $.ajax({
	 * 
	 * url:"rest/userprofile/updatetutorialstatus", type: "POST", data: {
	 * "inputData": JSON.stringify(inputData) }, datatype: "application/json",
	 * success:function(data){ if(data.error== null) {
	 * showToastMessage("Successfully updated"); }else{
	 * showErrorToastMessage(data.error.message); } }, error:function(data){
	...
	pgwSlideshow.reload({
		beforeSlide : function() {
			...
			console.log("currentSlide : " + currentSlide + ",totalSlide : "
					+ totalSlide);
		}
	});
...
function redirectToGettingToKnowLastPage() {
	...
	cont1
			.append("<div class='getting-to-know-hdr-txt'>I'm not finished with the tutorial</div>");

	var cont1btn1 = $('<div>').attr({
		"class" : "getting-to-know-btn float-right"
	}).html("I need to view the tutorial again").click(function() {
		paintGettingToKnowPage();
	});
	...
	cont2
			.append("<div class='getting-to-know-hdr-txt'>I'm finished with the tutorial, take me to</div>");

	var cont2btn1 = $('<div>').attr({
		"class" : "getting-to-know-btn float-right"
	}).html("Enter my loan information").on(
			'click',
			function() {

				removedKnwoNewFi = true;
				finishedTutorial(newfiObject.applicationKnowNewfi,
						"home.do#myLoan/my-application");
				newfiObject.applicationKnowNewfi = undefined;
	...
	var cont2btn2 = $('<div>').attr({
		"class" : "getting-to-know-btn float-right"
	}).html("View more rate options").on(
			'click',
			function() {

				removedKnwoNewFi = true;
				finishedTutorial(newfiObject.applicationKnowNewfi,
						"home.do#myLoan/lock-my-rate");
				newfiObject.applicationKnowNewfi = undefined;
			});

	var cont2btn3 = $('<div>').attr({
		"class" : "getting-to-know-btn float-right"
	}).html("Check out my loan progress").on(
			'click',
			function() {

				removedKnwoNewFi = true;
				finishedTutorial(newfiObject.applicationKnowNewfi,
						"home.do#myLoan/my-loan-progress");
				newfiObject.applicationKnowNewfi = undefined;
			});
	...
	if (flagToShowCompletPro) {
		cont2.append(cont2btn1).append(cont2btn2).append(cont2btn3).append(
				cont2btn4);
	} else {
...
function paintProfileCompleteStep2(user) {
	// On click of this next button the values are getting stored in the data
	// base
...
function getAboutMeDetailsContainer(user) {
	...
	if (user.firstName == "" || user.firstName == null) {
		nonEmptyValuesContainer.append(getEditableFormRow("First Name",
				user.firstName, "userFnameId"));
	} else {
		emptyValuesContainer.append(getNonEditableFormRow("First Name",
				user.firstName, "userFnameId"));
	}
	if (user.lastName == "" || user.lastName == null) {
		nonEmptyValuesContainer.append(getEditableFormRow("Last Name",
				user.lastName, "userlnameId"));
	} else {
		emptyValuesContainer.append(getNonEditableFormRow("Last Name",
				user.lastName, "userlnameId"));
	}
	// Primery Email id
	if (user.emailId == "" || user.emailId == null) {
		nonEmptyValuesContainer.append(getEditableFormRow("Email Id",
				user.emailId, "emailId"));
	} else {
		emptyValuesContainer.append(getNonEditableFormRow("Email Id",
				user.emailId, "emailId"));
	}
	// Secondry email id
	if (user.customerDetail.secEmailId == ""
			|| user.customerDetail.secEmailId == null) {
		nonEmptyValuesContainer.append(getEditableFormRow("Second Email Id",
				user.customerDetail.secEmailId, "secEmailId"));
	} else {
		emptyValuesContainer.append(getNonEditableFormRow("Second Email Id",
				user.customerDetail.secEmailId, "secEmailId"));
	}
	// Primery phone number
	if (user.phoneNumber == "" || user.phoneNumber == null) {
		nonEmptyValuesContainer.append(getEditableFormRow("Phone 1",
				user.phoneNumber, "priPhoneNumberId"));
	} else {
		emptyValuesContainer.append(getNonEditableFormRow("Phone 1",
				user.phoneNumber, "priPhoneNumberId"));
	}
	// secondry phone number
	if (user.customerDetail.secPhoneNumber == ""
			|| user.customerDetail.secPhoneNumber == null) {
		nonEmptyValuesContainer.append(getEditableFormRow("Phone 2",
				user.customerDetail.secPhoneNumber, "secPhoneNumberId"));
	} else {
		emptyValuesContainer.append(getNonEditableFormRow("Phone 2",
				user.customerDetail.secPhoneNumber, "secPhoneNumberId"));
	}
	// userCity
	if (user.customerDetail.addressCity == ""
			|| user.customerDetail.addressCity == null) {
		nonEmptyValuesContainer.append(getEditableFormRow("City",
				user.customerDetail.addressCity, "userCityId"));
	} else {
		emptyValuesContainer.append(getNonEditableFormRow("City",
				user.customerDetail.addressCity, "userCityId"));
	}
	// userState
	if (user.customerDetail.addressState == ""
			|| user.customerDetail.addressState == null) {
		nonEmptyValuesContainer.append(getEditableFormRow("State",
				user.customerDetail.addressState, "userStateId"));
	} else {
		emptyValuesContainer.append(getNonEditableFormRow("State",
				user.customerDetail.addressState, "userStateId"));
	}
	// userZipcode
	if (user.customerDetail.addressZipCode == ""
			|| user.customerDetail.addressZipCode == null) {
		nonEmptyValuesContainer.append(getEditableFormRow("Zipcode",
				user.customerDetail.addressZipCode, "userZipcodeId"));
	} else {
		emptyValuesContainer.append(getNonEditableFormRow("Zipcode",
				user.customerDetail.addressZipCode, "userZipcodeId"));
	}
	// dobId
	if (user.customerDetail.dateOfBirth == ""
			|| user.customerDetail.dateOfBirth == null
			|| user.customerDetail.dateOfBirth == '0l') {
		var comRow = getEditableFormRow("DOB", user.customerDetail.dateOfBirth,
				"dobId");
		...
		emptyValuesContainer.append(getNonEditableFormRow("DOB", $.datepicker
				.formatDate("mm/dd/yy", new Date(
						user.customerDetail.dateOfBirth)), "dobId"));
	}
	return container.append(nonEmptyValuesContainer).append(
			...
			.ajax({
				...
				success : function(data) {
					...
					if (status == "Error") {
						...
						element.html("Rate Lock Requested").unbind("click")
								.addClass("rateLockRequested");
						alert('loan is locked');
					}

					// TO:DO pass the data (json)which is coming from the
					// controller
...
>>>>>>> ff4409702530651c99016582769ef9e3a27743a3
		...
		success : function(data) {
			...
			/*
			 * if(data==""){ $('#center-panel-cont').html("Sorry, We could not
			 * find suitable products for you! One of our Loan officers will get
			 * in touch with you"); }else{
			 */
...
function fixAndLoakYourRatePage2(lqbData1, appUserDetails) {
	...
	$('#center-panel-cont').append(loanSummaryWrapper).append(
			closingCostWrapper);
...
function getSliderContainerForMobileScreen() {
	...
	if (loanType == "purchase") {
		...
		downPaymentBtnContainer.append(downPaymentlabel).append(
				downPaymentValue);
...
function getLoanSummaryWrapper(lqbData, appUserDetails) {
	...
	parentWrapper.append(header).append(container).append(rateWrapper).append(
			bottomText);
...
function getLoanSummaryContainerPurchase(lqbData, appUserDetails) {
	...
	var lcRow2 = getLoanSummaryRow("Loan Program", rateVoObj.yearData
			+ " Years Fixed", "loanprogramId");
	// var lcRow3 = getLoanSummaryRow("Loan Amount", loanAmount);
	var lcRow3 = getLoanAmountRowPurchase("Loan Amount", showValue(loanAmount),
			"loanAmount", "Purchase Amount", showValue(housePrice),
			" Down Payment", showValue(downPayment), false);
	...
	// var lcRow6 = getLoanSummaryLastRow("Estimated<br/>Closing Cost",
	// showValue(rateVO[index].closingCost), "closingCostId");
	...
	if (appUserDetails.purchaseDetails.livingSituation != "homeOwner")
		rcRow1 = getLoanSummaryRow("Current Rental Payment",
				showValue(appUserDetails.monthlyRent), "monthlyPaymentId");

	var rcRow2 = getLoanSummaryRow("Proposed Principal & Interest",
			showValue(rateVoObj.payment), "principalIntId");
	var rcRow3 = getLoanSummaryRowCalculateBtn("Tax", showValue(tax),
			"calTaxID", "calTaxID2", appUserDetails);
	rcRow3.addClass("no-border-bottom");
	var rcRow4 = getLoanSummaryRowCalculateBtn("Insurance",
			showValue(Insurance), "CalInsuranceID", "CalInsuranceID2",
			appUserDetails);
	// var rcRow5 = getLoanSummaryTextRow("Your tax and insurance payment above
	// will be included with your principal & interest payment");
	// var rcRow6 = getLoanSummaryLastRow("Estimated<br/>Monthly Payment",
	// showValue(totalEstMonthlyPayment),"totalEstMonthlyPaymentId");
	...
	var bottomLcRow = getLoanSummaryLastRow("Estimated<br/>Closing Cost",
			showValue(rateVoObj.closingCost), "closingCostId");
	...
	var bottomRcRow = getLoanSummaryLastRow("Estimated<br/>Monthly Payment",
			showValue(totalEstMonthlyPayment), "totalEstMonthlyPaymentId");
...
function getLoanSummaryContainerRefinance(lqbData, appUserDetails) {
	...
	if (appUserDetails.refinancedetails.refinanceOption == "REFLMP")
		refinanceOpt = "Lower monthly payment";
	if (appUserDetails.refinancedetails.refinanceOption == "REFMF")
		refinanceOpt = "Pay off mortgage faster";
	...
	var monthlyPaymentDifference = (Math
			.abs(principalInterest - monthlyPayment));
	...
	if (appUserDetails.refinancedetails.refinanceOption != "REFCO")
		var lcRow4 = getLoanAmountRow("Loan Amount", showValue(loanAmount),
				"loanAmount");
	else
		var lcRow4 = getLoanAmountRowPurchase("Loan Amount",
				showValue(loanAmount), "loanAmount", "Current Loan Amout",
				showValue(currentMortgageBalance), "Cashout",
				showValue(cashTakeOut), true);

	var lcRow5 = getLoanSummaryRow("APR", rateVoObj.APR + " %", "lockrateaprid");
	// var lcRow6 = getLoanSummaryLastRow("Estimated<br/>Closing Cost",
	// showValue(rateVO[index].closingCost), "lockClosingCost");
	leftCol.append(lcRow1).append(lcRow2).append(lcRow3).append(lcRow4).append(
			lcRow5);
	...
	var rcRow1 = getLoanSummaryRow("Proposed Principal & Interest",
			showValue(principalInterest), "principalIntId");
	var rcRow2 = getLoanSummaryRowCalculateBtn("Tax", showValue(tax),
			"calTaxID", "calTaxID2", appUserDetails);
	rcRow2.addClass("no-border-bottom");
	var rcRow3 = getLoanSummaryRowCalculateBtn("Insurance",
			showValue(Insurance), "CalInsuranceID", "CalInsuranceID2",
			appUserDetails);
	// var rcRow4 = getLoanSummaryTextRow("Your tax and insurance payment above
	// will be included with your principal & interest payment");
	// var rcRow5 = getLoanSummaryRow("Total Est. Monthly Payment", "$
	// 3,298.40");
	var rcRow6 = getLoanSummaryRow("Current Monthly Payment",
			showValue(monthlyPayment), "monthlyPaymentId");
	...
	var bottomLcRow = getLoanSummaryLastRow("Estimated<br/>Closing Cost",
			showValue(rateVoObj.closingCost), "lockClosingCost");
	...
	var bottomRcRow = getLoanSummaryLastRow("Total Est.<br/>Monthly Payment",
			showValue(totalEstMonthlyPayment), "totalEstMonthlyPaymentId");
	bottomRightCol.append(bottomRcRow);
	var hgLow = "";
	...
	var rcRow7 = getLoanSummaryLastRow('This Monthly<br/> Payment is ' + hgLow
			+ ' by', showValue(monthlyPaymentDifference),
			"monthlyPaymentDifferenceId");
...
function getLoanAmountRow(desc, detail, id, row1Desc, row1Val, row2Desc,
		row2Val) {
	...
	var input = $('<input>').attr({
		 ...
		 * .on('keyup',function(e){ if(e.which == 27){ $(this).blur(); } });
		 */
...
function getLoanSummaryRowCalculateBtn(desc, detail, id, id2, appUserDetails) {
	...
	/*
	 * var col2Txt = $('<div>').attr({ "class" : "calculate-btn", "id":id
	 * 
	 * }).html(detail) .bind('click',function(event){ var prevVal =
	 * $(this).text();
	 * 
	 * if(prevVal!="Edit"){
	 * $(this).next('input').show().focus().val(prevVal).numericInput({
	 * allowFloat :false, allowNegative: false }); }else{
	 * $(this).next('input').show().focus().val(""); } $(this).hide();
	 * 
	 * });
	 */
	...
	var inputBox = $('<input>')
			...
			.bind(
					...
					function(e) {
						...
						/*
						 * if(e.which == 27){ var prevVal =
						 * $(this).prev('.calculate-btn').text();
						...
						var taxesTemp = 0;
						if ($('#calTaxID2').val() != "")
							taxesTemp = parseFloat(removedDoller(removedComma($(
									'#calTaxID2').val())));

						var InsuranceTemp = 0;
						if ($('#CalInsuranceID2').val() != "")
							InsuranceTemp = parseFloat(removedDoller(removedComma($(
									'#CalInsuranceID2').val())));
						...
						var principalInt = parseFloat(removedDoller(removedComma($(
								'#principalIntId').text())));

						var monthlyPaymentDifferenceTemp = Math
								.abs(principalInt - monthlyPayment);
						var monthlyPaymentDifference = monthlyPaymentDifferenceTemp
								.toFixed(3);

						var totalEstMonthlyPaymentId = (principalInt + investment)
								.toFixed(3);

						$('#monthlyPaymentId').text(showValue(monthlyPayment));
						$('#monthlyPaymentDifferenceId').text(
								showValue(monthlyPaymentDifference));
						$('#totalEstMonthlyPaymentId').text(
								showValue(totalEstMonthlyPaymentId));
						...
						$(itm).find(".loan-summary-col-desc")
								.html(
										'This Monthly<br/> Payment is ' + hgLow
												+ ' by');
						if (hgLow == '<font color="green"><b>Lower</b></font>') {
							if (!($("#monthlyPaymentDifferenceId")
									.hasClass("loan-summary-green-col-detail"))) {
								$("#monthlyPaymentDifferenceId").removeClass(
										"loan-summary-red-col-detail");
								$("#monthlyPaymentDifferenceId").addClass(
										"loan-summary-green-col-detail");
							}
						} else {
							if (!($("#monthlyPaymentDifferenceId")
									.hasClass("loan-summary-red-col-detail"))) {
								$("#monthlyPaymentDifferenceId").removeClass(
										"loan-summary-green-col-detail");
								$("#monthlyPaymentDifferenceId").addClass(
										"loan-summary-red-col-detail");
...
function populateClosingCostHolder(inputObject) {
	...
	for (var attrCount = 0; attrCount <= dynamicList.length; attrCount++) {
		...
		if (typeof inputObject[currentAttribute] == "object"
				&& Object.keys(inputObject[currentAttribute]).length > 0) {
...
function setClosingCostContainerValues() {
	if (appUserDetails != undefined) {
		closingCostHolder.loanType = (appUserDetails.loanType != undefined ? appUserDetails.loanType.loanTypeCd
				: "PUR")
	} else {
		closingCostHolder.loanType = "PUR";
	}
...
function getClosingCostSummaryContainer(valueSet) {
	...
	if (valueSet.dummyData) {
		if (typeof (newfiObject) !== 'undefined')
			parentWrapper
					.html(getHeaderText("Sorry, We could not find suitable products for you! One of our Loan officers will get in touch with you"));
		else {
			parentWrapper
					.html(getHeaderText("Sorry, We could not find suitable products for you!"));
		}
	} else {
		var header = getClosingCostHeader("Closing Cost Summary");
		var descText = getHeaderText("Based on the information you have provided, below is a summary of your estimated closing costs:");
		...
		var topContainer = getClosingCostTopConatiner();
		var bottomContainer = getClosingCostBottomConatiner();
		parentWrapper.append(header).append(descText).append(topContainer)
...
function getClosingCostTopConatiner() {
	...
	var row1Con1 = getClosingCostContainerRow(1,
			getClosingCostLabel("Lender Fee"), "$ 1,495.00");
	var row2Con1 = getClosingCostContainerRow(
			2,
			getClosingCostLabel("This is your cost or credit based on rate selected"),
			"$ 5,128.75");
	var row3Con1 = getClosingCostContainerLastRow(3,
			getClosingCostLabel("Estimated Lender Costs"), "$ 6,623.75");
	...
	var row1Con2 = getClosingCostContainerRow(1,
			getClosingCostLabel("Appraisal Fee"), "$ 455.00");
	var row2Con2 = getClosingCostContainerRow(2,
			getClosingCostLabel("Credit Report"), "$ 455.00");
	var row3Con2 = getClosingCostContainerRow(3,
			getClosingCostLabel("Flood Certification"), "$ 455.00");
	var row4Con2 = getClosingCostContainerRow(4,
			getClosingCostLabel("Wire Fee"), "$ 455.00");
	var row4_1Con2;
	if (closingCostHolder.loanType && closingCostHolder.loanType == "PUR")
		row4_1Con2 = getClosingCostContainerRow(5,
				getClosingCostLabel("Owners Title Insurance"), "$ 450.00");
	var row5Con2 = getClosingCostContainerRow(5,
			getClosingCostLabel("Lenders Title Insurance"), "$ 450.00");
	var row6Con2 = getClosingCostContainerRow(6,
			getClosingCostLabel("Closing/Escrow Fee"), "$ 500.00");
	var row7Con2 = getClosingCostContainerRow(7,
			getClosingCostLabel("Recording Fee"), "$ 107.00");
	var row8Con2;
	if (closingCostHolder.loanType && closingCostHolder.loanType == "PUR")
		row8Con2 = getClosingCostContainerRow(8,
				getClosingCostLabel("City/County Tax stamps"), "$ 107.00");
	var row9Con2 = getClosingCostContainerLastRow(9,
			getClosingCostLabel("Total Estimated Third Party Costs"),
			"$ 1,562.00");
	container2.append(headerCon2).append(row1Con2).append(row2Con2).append(
			row3Con2).append(row4Con2).append(row4_1Con2).append(row5Con2)
			.append(row6Con2).append(row7Con2).append(row8Con2)
			.append(row9Con2);
	...
	var row1Con3 = getClosingCostContainerRowWithSubText(1,
			getClosingCostLabel("Interest"), "$ 699.40", "");
	var row2Con3 = getClosingCostContainerRow(2,
			getClosingCostLabel("Homeowners Insurance"), "$ 455.00");
	var row3Con3 = getClosingCostContainerLastRow(3,
			getClosingCostLabel("Total Prepaids"), "$ 699.40");
	var row10Con3 = getClosingCostContainerLastRow(10,
			getClosingCostLabel("Total Estimated Closing Cost"), "$ 8,162.00");

	container3.append(headerCon3).append(row1Con3).append(row2Con3).append(
			row3Con3).append(row10Con3);
...
function getClosingCostBottomConatiner() {
	...
	var row1Con2 = getClosingCostContainerRowWithSubText(1,
			getClosingCostLabel("Tax Reserve - Estimated 2 Month(s)"),
			"$ 1,072.00", "(Varies based on calendar month of closing)");
	var row2Con2 = getClosingCostContainerRowWithSubText(
			2,
			getClosingCostLabel("Homeowners Insurance Reserve - Estimated 2 Month(s)"),
			"$ 1,072.00", "(Provided you have 6 months of remaining coverage)");
	var row4Con2 = getClosingCostContainerLastRow(
			4,
			getClosingCostLabel("Total Estimated Reserves Deposited in Escrow Account"),
			"$ 3,216.00");
	container2.append(headerCon2).append(row1Con2).append(row2Con2).append(
			row4Con2);
	var bottomSubText = $('<div>')
			...
			.html(
					"Note: Taxes for 1st and 2nd installments must be paid or will be collected at closing.");
...
function getDocumentContainer() {
	...
	for (i in listUploadedFiles) {
		if (listUploadedFiles[i].needType == undefined
			...
			// Unassigned file and coming from LQB
			if (listUploadedFiles[i].isMiscellaneous == false
					&& userIsInternal()) {
...
/*
 * function appendAlertNotificationPopup() { var alertWrapper = $('<div>').attr({
 * "id": "alert-popup-wrapper", "class": "alert-popup-wrapper" }); var row1 =
 * getAlertNotificationRow("Salaried-W2-forms", "2hr ago", false); var row2 =
 * getAlertNotificationRow("Salaried-W2-forms", "2hr ago", true); var row3 =
 * getAlertNotificationRow("Salaried-W2-forms", "2hr ago", false); var row4 =
 * getAlertNotificationRow("Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum",
 * "2hr ago", false); var row5 = getAlertNotificationRow("Lorem Ipsum Lorem
 * Ipsum Lorem Ipsum Lorem Ipsum", "2hr ago", false); var row6 =
 * getAlertNotificationRow("Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum",
 * "2hr ago", false);
 ...
 * $('#alert-popup-wrapper').perfectScrollbar({ suppressScrollX : true }); }
 */
		...
		ajaxRequest(
				...
				function(response) {
					if (response.error) {
						...
						if (status
								&& newfiObject.user.userRole.roleCd != "REALTOR") {
							element.addClass("rate-btn");
							if (!rateLockRequestedFlag) {
								element.html("Request Rate Lock").on('click',
										function(event) {

											lockLoanRate(lockratedata);
										});
							} else {
								element.html("Rate Lock Requested").unbind(
										"click").addClass("rateLockRequested");
							}
						} else {
							element.addClass("rate-btn");
							element
									.html("Contact Your Loan Advisor")
									.on(
											'click',
											function() {
												// changeLeftPanel(1);
...
function getYearSlider(LQBResponse, appUserDetails) {
	...
	for (var i = 0; i < yearValues.length; i++) {
				...
				.bind(
						...
						function(event) {
							if (!$(this).hasClass('yr-slider-icon-selected')) {
								$('.yr-grid-cont .yr-slider-icon').removeClass(
										'yr-slider-icon-selected');
								$(this).addClass('yr-slider-icon-selected');
								$('.yr-grid-cont .yr-grid-item-selected')
										.hide();
								$('.yr-grid-cont .yr-grid-item').show();
								$(this).parent().find('.yr-grid-item').hide();
								$(this).parent().find('.yr-grid-item-selected')
										.show();
								$('#rate-slider-cont').find('.rt-slider')
										.remove();
								var rateSlider = getRatSlider(
										event.data.ratesArray, appUserDetails,
										event.data.year);
								...
								$('#lockrateaprid')
										.html(
												event.data.ratesArray[index].APR
														+ " %");
								$('#lockClosingCost')
										.html(
												showValue(event.data.ratesArray[index].closingCost));
								$('#lockInterestRate').html(
										event.data.ratesArray[index].teaserRate
												+ " %");
								$('#principalIntId')
										.html(
												showValue(event.data.ratesArray[index].payment));

								if (event.data.year == '5'
										|| event.data.year == "7")
									$('#loanprogramId').html(
											event.data.year + " Year ARM");
								else
									$('#loanprogramId').html(
											event.data.year + " Year Fixed");
		...
		gridItemCont.append(selectIcon).append(gridItem).append(
				gridItemSelected);
...
function getRateSliderCont(LQBResponse, appUserDetails) {
	...
	var silderCont = getRatSlider(rateArray, appUserDetails,
			yearValues[yearValues.length - 1].value);
	return wrapper.append(headerTxt).append(silderCont);
}
			...
			.slider(
					{
						...
						change : function(event, ui) {
							...
							$('#lockrateaprid').html(
									gridArray[ui.value].APR + " %");
							$('#lockClosingCost').html(
									showValue(gridArray[ui.value].closingCost));
							$('#lockInterestRate').html(
									parseFloat(gridArray[ui.value].teaserRate)
											.toFixed(3)
											+ " %");
							$('#principalIntId').html(
									showValue(gridArray[ui.value].payment));
...
function lockRateCalculation(appUserDetails) {
	...
	var taxesTemp = 0;
	var InsuranceTemp = 0;
	var principalInterest = parseFloat(removedDoller(removedComma($(
			'#principalIntId').text())));
...
function getLoanAmountRowPurchase(desc, detail, id, row1Desc, row1Val,
		row2Desc, row2Val, cashOutCheck, path) {
	...
	var col2row1 = $('<input>').attr({
		"class" : "loan-summary-sub-col-detail float-left",
		"id" : "firstInput"
	}).val(showValue(row1Val)).keydown(function() {
		$(this).maskMoney({
			thousands : ',',
			decimal : '.',
			allowZero : true,
			...
			.attr(
					{
						...
						"id" : "secondInput"
					}).val(showValue(row2Val)).keydown(function() {
				$(this).maskMoney({
					thousands : ',',
					decimal : '.',
					allowZero : true,
...
function saveTaxAndInsurance() {
	...
	appUserDetails.propertyTypeMaster.propertyInsuranceCost = $(
			'#CalInsuranceID2').val();
