...
DELETE FROM `newfi_schema`.`workflowitemmaster` WHERE parent_workflow_item_master IS NOT NULL;
DELETE FROM `newfi_schema`.`workflowitemmaster` WHERE parent_workflow_item_master IS  NULL;
DELETE FROM `newfi_schema`.`workflowmaster` WHERE id>0;

DELETE FROM `newfi_schema`.`workflowtaskconfigmaster` where id>0;

DELETE FROM `newfi_schema`.`loanmilestonemaster` where id>0;

insert  into `newfi_schema`.`loanmilestonemaster`(`id`,`name`,`description`,`loan_type`,`milestone_validator`) values (1,'1003','1003 Completion',1,NULL),(2,'AUSUW','Automated Underwriting',1,NULL),(3,'QC','QC Decision',1,NULL),(4,'LM_DECISION','Underwriting',1,NULL),(5,'DISCLOSURE','Disclosure',1,NULL),(6,'APPRAISAL','Appraisal',1,NULL),(7,'UW','Underwriting',1,NULL),(8,'LOAN_CLOSURE','Loan Closure',1,NULL);
...
LOCK TABLES `newfi_schema`.`workflowitemmaster` WRITE;
